...
var Session = {
        init: function (req, res, store, options) {
          ...
          this._expire  = new Date(Date.now() + (this.options.expire * 1000))

          cookieOptions = { expires : this._expire, signed: !!this.keys }
      ...
      , del: function (key, callback) {
          ...
          this.store.del(this.id, key, this._expire, callback || noop)
        }
      ...
      , set: function (key, value, callback) {
          ...
          this.store.set(this.id, key, value, this._expire, callback || noop)
        }
      ...
      , get: function (key, callback) {
          ...
          if (!callback)
            return this.store.extend(this.id, this._expire)

          this.store.get(this.id, key, this._expire, callback)
        }
      ...
      , getAll: function (callback) {
          ...
          this.store.getAll(this.id, this._expire, callback)
        }
    }
